...
FTB_Button.prototype.Initialize = function() {
    ...
    FTB_AddEvent(this.td, "click", function() {
        if (FTB_Browser.isIE) document.getElementById(id).button.Click();
        else this.button.Click();
    });
    FTB_AddEvent(this.td, "mouseover", function() {
        if (FTB_Browser.isIE) document.getElementById(id).button.MouseOver();
        else this.button.MouseOver();
    });
    FTB_AddEvent(this.td, "mouseout", function() {
        if (FTB_Browser.isIE) document.getElementById(id).button.MouseOut();
        else this.button.MouseOut();
    });
};
...
function FTB_DropDownList(id, commandIdentifier, customAction, customStateQuery, customEnabled) {
    ...
    if (this.list) {
        ...
        FTB_AddEvent(this.list, "change", function() {
            if (FTB_Browser.isIE) document.getElementById(id).dropDownList.Select();
            else this.dropDownList.Select();
        });
    } else {
