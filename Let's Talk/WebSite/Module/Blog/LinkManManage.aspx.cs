...
public partial class Module_Blog_MessageList : Page
{
    private string ID;
        ...
        switch (search)
        {
            case "联系人ID":
                ...
                da.BindData(GridView1,
                            "Select * From tb_Message Where MessageID  Like  '%" + txtKey.Text + "%' and BlogID='" + ID +
                            "'");
                break;
            case "姓名":

                da.BindData(GridView1,
                            "Select * From tb_Message Where FriendName  Like  '%" + txtKey.Text + "%' and BlogID='" + ID +
                            "'");
                break;
            case "QQ":

                da.BindData(GridView1,
                            "Select * From tb_Message Where QQ  Like  '%" + txtKey.Text + "%'  and BlogID='" + ID + "'");
                break;
            default:
