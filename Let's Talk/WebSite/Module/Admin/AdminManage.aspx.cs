...
public partial class Module_Admin_Append : Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            gvAdmin.DataSource = da.ExceDS("select * from tb_Admin");
            gvAdmin.DataKeyNames = new[] {"id"};
            gvAdmin.DataBind();
        }
    }
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        {
            case "管理员ID":
                da.BindData(gvAdmin, "Select * From tb_Admin Where ID  Like  '%" + txtKey.Text + "%'");
                break;
            case "用户名":
                da.BindData(gvAdmin, "Select * From tb_Admin Where UserName  Like  '%" + txtKey.Text + "%'");
                break;
            case "QQ":
                da.BindData(gvAdmin, "Select * From tb_Admin Where QQ  Like  '%" + txtKey.Text + "%'");
                break;
            default:
    ...
    {
        Response.Redirect(string.Format("../../Images/Pictures/Dialog.aspx?rif=Images/Pictures&cif=Images/Pictures\\"),
                          true);
    }
}